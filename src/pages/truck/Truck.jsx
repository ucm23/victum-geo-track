    ...
    useEffect(() => {
        async function getTodos() {
            ...
            if (plate) {
                ...
                response = await supabase.rpc('get_trucks_by_company_', { _company_id_: company_id, _page: page, _page_size: pageSize });
            }
    ...
    const renderItem = ({ item, index }) => {
        return (
            <tr key={index} className={'table-bg-by-index'}>
                ...
                <td>{item?.no_econ}</td>
                <td>{item?.no_serie}</td>
                <td className='th-center'>{item?.in_function ? <Badge status="processing" /> : <Badge status="warning" />}</td>
                <td className='th-center'><ChakraBadge className='chakra-badge-status' variant='subtle' colorScheme={getColorStatus(item?.status?.id)}>{item?.status?.name}</ChakraBadge></td>
                <td>
                    <Dropdown menu={{
                        items: [
                            {
                                ...
                                <table>
                                    <thead className="cabecera">
                                        <tr>
                                            ...
                                            <th>NO ECON</th>
                                            <th>NO SERIE</th>
                                            <th className='th-center'>ACTIVO</th>
